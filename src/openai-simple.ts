...
/**
 * Executa um workflow do ChatKit usando a implementação que funciona
...
export async function runChatKitWorkflow(
  ...
  try {
    // Usar diretamente a API de chat da OpenAI em vez do ChatKit
    const messages: ConversationMessage[] = [
      {
        role: "system",
        content: `Você é um assistente útil do ZPRO. Execute o workflow ${workflowId} conforme solicitado pelo usuário. 
...
- Se for uma pergunta sobre ZPRO, responda com informações relevantes
...
export async function sendMessageToOpenAI(
  ...
  return await runChatKitWorkflow(defaultWorkflowId, message, context, conversationHistory);
}
